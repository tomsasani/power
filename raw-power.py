...
def depth_heap(mdk, cutoff=10):
    """
    m, d, k are iterables returned from a pytabix query.
    """
    ...
    while len(heap) > 0:
        ...
        yn = all(h.depth >= cutoff for h in heap)
...
def heap_thresh(region, trio, return_dict, cutoff):
    """
...
def evaluate_dp_thresh(region, trio, return_dict, cutoff):
    ...
    shared_bases = ne.evaluate("ka >= cutoff & ma >= cutoff & da >= cutoff")
    shared_bases.sum()
    return_dict[str(region)] = str(shared_bases)
...
def main(args):
    ...
    for trio in generate_trios(args.ped):
        k,m,d = trio
        k,m,d = (str(x) for x in (k,m,d))#str(k), str(m), str(d)
        ...
        with Pool(processes=args.processes) as pool:
            pool.starmap(evaluate_dp_thresh, iterable)
